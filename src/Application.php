...
class Application
{
    ...
    public function LoadApplicationConfiguration($routing = null, $app = null)
    {
        $routing();
        $app();
    }
    ...
    public function RunControllerAction($ControllerAction, $Parameters)
    {
    ...
    public function BeforeAction()
    {
        if (!empty($this->Router->RouteInfo->BeforeAction)) {
            foreach ($this->Router->RouteInfo->BeforeAction as $event_handler) {
                ...
                } elseif (is_callable($eh['function'])) {
                    throw new \Exception('Not implemented!');
                }
                if (isset($result)) {
                    if (is_object($result)) {
    ...
    public function Run()
    {
        ...
        if ($action = $this->Router->match($this->Request)) {
            ...
            } elseif (is_object($action) && is_a($action, 'L2_controler_info')) {
                // not implemented yet.
                throw new \Exception('Not implemented!');
            ///$action->runControlerAction($this->routing->param);
            }
            // array with controller, and action -> run the controller action
            elseif (is_array($action) && array_key_exists('controller', $action) && array_key_exists('action', $action)) {
